...
def get_features(img,bbox):
    ...
    roi_start = (np.min(bbox[:,0]),np.min(bbox[:,1]))
    roi_w = np.max(bbox[:,0]) - np.min(bbox[:,0])
    roi_h = np.max(bbox[:,1]) - np.min(bbox[:,1])
    roi = img[roi_start[0]:roi_start+row_w, roi_start[1]:roi_start+roi_h]
    fps = corner_peaks(corner_shi_tomasi(square), min_distance=1)
    x = fps[:,0] + roi_w
    y = fps[:,1] + roi_h
    return x,y
